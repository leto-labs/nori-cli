...
impl acp::Agent for MockAgent {
    ...
    async fn new_session(
        ...
        self.session_configs
            .borrow_mut()
            .insert(session_key, config.clone());
    ...
    async fn prompt(
        ...
        if std::env::var("MOCK_AGENT_SEND_CONFIG_UPDATE").is_ok() {
            let updated = {
                let mut sessions = self.session_configs.borrow_mut();
                let state = sessions
                    .entry(session_id.to_string())
                    .or_insert_with(Self::default_session_config);
                state.thought_level = Some("high".to_string());
    ...
    async fn set_session_config_option(
        ...
        let response_options = {
            ...
            match args.config_id.to_string().as_str() {
                "model" => {
                    state.model_id = value.to_string();
                ...
                "thought_level" => {
                    state.thought_level = Some(value.to_string());
                }
                "speed" => {
